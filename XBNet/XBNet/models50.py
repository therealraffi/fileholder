...
class XBNETClassifier50(torch.nn.Module):
    '''
    ...
    def __init__(self, X_values, y_values, num_layers, num_layers_boosted=1,
        ...
        self.xg = XGBClassifier(n_estimators=50)
    ...
    def base_tree(self):
        ...
        '''
        self.temp1 = XGBClassifier(n_estimators=50).fit(self.X, self.y,eval_metric="mlogloss").feature_importances_
...
class XBNETRegressor(torch.nn.Module):
    ...
    def __init__(self, X_values, y_values, num_layers, num_layers_boosted=1):
        ...
        self.xg = XGBRegressor(n_estimators=50)
    ...
    def base_tree(self):
        ...
        Instantiates and trains a XGBRegressor on the first layer of the neural network to set its feature importances
         as the weights of the layer
        '''
        self.temp1 = XGBRegressor(n_estimators=50).fit(self.X, self.y,eval_metric="mlogloss").feature_importances_
        self.temp = self.temp1
        for i in range(1, self.input_out_dim):
            self.temp = np.column_stack((self.temp, self.temp1))
